...
public:
	...
	void GENRL_KickOutOfRestaurant(Time *cusTime)
	{
		...
		if (cusTime->isSittingOnTable)
		{
			...
			else if (Table_CurrentSize > 1)
			{
				...
				if (kickedCustomer->energy > 0)
				{
					Table_Customer = kickedCustomer->next;
				}
				else if (kickedCustomer->energy < 0)
				{
					Table_Customer = kickedCustomer->prev;
				}
				Table_CurrentSize--;
	...
	void GENRL_MoveCustomer_WaitlistToTable()
	{
		while (Table_CurrentSize < MAXSIZE && WaitList_CurrentSize > 0)
		{
			...
			else if (Table_CurrentSize >= MAXSIZE / 2)
			{
				...
				if (RESwithoutAbsolute > 0)
				{
					RED_addCustomer_Clockwise(movingCustomer, maxDiffCustomer);
				}
				else
				{
					RED_addCustomer_counterClockwise(movingCustomer, maxDiffCustomer);
				}
			}
			else
			{
				if (movingCustomer->energy >= Table_Customer->energy)
				{
					RED_addCustomer_Clockwise(movingCustomer, Table_Customer);
				}
				else
				{
					RED_addCustomer_counterClockwise(movingCustomer, Table_Customer);
				}
			}
	...
	void RED(string name, int energy)
	{
		...
		else if (Table_CurrentSize >= MAXSIZE / 2)
		{
			...
			if (RESwithoutAbsolute > 0)
			{
				RED_addCustomer_Clockwise(newCustomer, maxDiffCustomer);
			}
			// nếu RES < 0 -> thêm ngược chiều kim đồng hồ
			else if (RESwithoutAbsolute < 0)
			{
				RED_addCustomer_counterClockwise(newCustomer, maxDiffCustomer);
			}
		...
		else
		{
			...
			if (energy >= Table_Customer->energy)
			{
				RED_addCustomer_Clockwise(newCustomer, Table_Customer);
			}
			// nếu năng lượng bé hơn -> thêm ngược chiều kim đồng hồ
			else
			{
				RED_addCustomer_counterClockwise(newCustomer, Table_Customer);
			}
	...
	{ // OK
		...
		{
			LIGHT_PrintTablelist_Clockwise();
		}
		// in thông tin khách trong bàn ngược chiều kim đồng hồ
		else if (num < 0)
		{
			LIGHT_PrintTablelist_counterClockwise();
		}
